...
//	LocBairroExcelBean locBairrosExcelBean;
    ...
    public EstruturaFisicaExcelBean()
    {
        ...
        default_filename = Defss.CAMINHO_LOCALIDADE;
    }
    ...
    {
        default_filename = grlExcelPathFacade.getCurrentGrlExcelPath() + Defss.NOME_FILE_ESTRUTURA_FISICA;
...
//System.out.println("2: CarregarTabelas.carregarTabelas()");                        
    ...
    public EstruturaFisica lerCampos(HSSFRow row) throws ExcepcaoCarregamentoTabelasExcel
    {
        ...
        for (int i = 0; i < 3; i++)
        {
            ...
            if (HSSFCellUtilitiess.isCellEmpty(cells[i]))
            {
                return null;
            }
        }
        pkEstruturaFisica = HSSFCellUtilitiess.getStringCellValue(cells[0]);
        designacao = HSSFCellUtilitiess.getStringCellValue(cells[1]);
        fkLocalidade = HSSFCellUtilitiess.getStringCellValue(cells[2]);
